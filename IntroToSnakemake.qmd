...
- Instead of a `shell:` block that gives the commands to be run on the terminal, this rule uses a `script:` block that lists a (not yet existing) script to run. Actually, it is possible to put any shell command into a `shell:` block, including one that runs a Python or an R script, like `python myscript.py` or `R CMD BATCH myscript.R output.txt`. While these approaches might be preferable in certain cases, for example because they would allow you to capture the R session including console input and output, they will be more difficult to implement for certain aspects, like getting parameters from `snakemake` (see below).
