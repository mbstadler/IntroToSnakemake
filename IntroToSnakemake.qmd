...
![snakemake overview](snakemake_overview.png)
...
At the moment, there is nothing in our `Snakefile` that needs a file of the form `"data_processed/*.counts.txt"`, and if you were to run `snakemake`, there would still be nothing to do.

Let's add the following additional rule above the `download_data` rule: 
...
- the pipe character `|` connecting two tools by feeding the `STDOUT` from the left to the `STDIN` from the right tool
...
Let's add a rule to the `Snakefile`, for exapmle under the `download_data` rule, that downloads a stop word list from the internet:
...
We can now clearly see that `romeo` and `juliet` are two very important characters in this text, and also that it is about love. The fact that the counts for the two main characters' names (302 + 182 = 484) is a bit below the cound obtained using `grep` above (514) likely means that there are occurrences of either name that match the regular expression `(romeo|juliet)` but are not identical to just the name, as for example `"Juliet's"` or `"Romeo's"`.
...
Finally, create a now rule `report_wordcloud` at the end of our `Snakefile`:
...
rule report_wordcloud:
    ...
    params:
        outdir = lambda wildcards, output: topdir + "/" + os.path.dirname(output["html"]),
    ...
    shell:
        ...
        Rscript -e "quarto::quarto_render('$(basename {input.qmd})', execute_params = list(
            topdir='{topdir}',
            txtfile='{input.txt}',
            stopwordfile='{input.stopword}'), output_file='{params.outname}')"
        '''
```
:::
...
- [Orjuela et al. 2019, "ARMOR: An Automated Reproducible MOdular Workflow for Preprocessing and Differential Analysis of RNA-seq Data"](https://pmc.ncbi.nlm.nih.gov/articles/PMC6643886/)
